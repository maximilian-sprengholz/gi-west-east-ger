...
// Update 27.11.2017 by Maximilian Sprengholz:
// Styling and naming of graph possible via passing arguments in the program command

// Update 11.02.2020: Implemented weighting using aweights (careful, not really tested!)
...
program DCdensity, rclass
{
  ...
  syntax varname(numeric) [if/] [in/], breakpoint(real) GENerate(string) ///
    [ b(real 0) h(real 0) at(string) aweight(varname) graphname(string) noGRaph ///
	ylabel(string) xlabel(string) title(string) subtitle(string) ///
	ytitle(string) xtitle(string) xsize(string) ysize(string) scheme(string) ///
	xboundlo(string) xboundhi(string) ] // NEW: aweight(varname)
  ...
  mata: DCdensitysub("`R'", "`touse'", `breakpoint', `b', `h', `verbose', "`cellmpname'", "`cellvalname'", ///
                     "`evalname'", "`cellsmname'", "`cellsmsename'", "`at'", "`aweight'")
  ...
  if ("`graph'"!="nograph") {
	...
	*** if boundaries specified for displaying the scatterplot
	if ("`xboundlo'" == "" & "`xboundhi'" == "") {
		local boundaries = ""
	}
	else if ("`xboundlo'" != "" & "`xboundhi'" == "") {
		local boundaries = "if `cellmpname'>=`xboundlo'"
	}
	else if ("`xboundlo'" == "" & "`xboundhi'" != "") {
		local boundaries = "if `cellmpname'<=`xboundhi'"
	}
	else {
		local boundaries = "if `cellmpname'>=`xboundlo' & `cellmpname'<=`xboundhi'"
	}
    gr twoway (scatter `cellvalname' `cellmpname' `boundaries', msymbol(circle_hollow) mcolor(gray))           ///
      (line `cellsmname' `evalname' if `evalname' < `breakpoint', lcolor(black) lwidth(0.3) lpattern(dash))   ///
        (line `cellsmname' `evalname' if `evalname' > `breakpoint', lcolor(black) lwidth(0.3) lpattern(dash)),   ///
                  xline(`breakpoint', lcolor(black) lwidth(0.3)) legend(off) scheme(`scheme') ///
		  graphregion(color(white) lcolor(black) margin(medlarge)) bgcolor(white) xsize(`xsize') ysize(`ysize') ///
		  xlabel(`xlabel') xscale(titlegap(2)) yscale(noline titlegap(3)) ylabel(`ylabel', angle(horizontal)) ///
...
void DCdensitysub(string scalar runvar, string scalar tousevar, real scalar c, real scalar b, ///
                  ...
                  string scalar atname, string scalar aweight) {
  ...
  //			 NEW: aweight - user provided weights to weight histogram
  ...
  real scalar loopover						// NEW: was undeclared, see below
  real colvector weight						// NEW: vector to hold obs. weights
  real scalar weightsum						// NEW: sum of weights

  //Set up histogram grid

  st_view(run, ., runvar, tousevar)     //view of running variable--only observations for which `touse'=1

  //Get summary stats on running variable (NEW: apply weights if requested!)
  if (aweight!="") {
	  statacom = "quietly summarize " + runvar + " if " + tousevar + " [aw=" + aweight + "] , det"
  ...
  /*
   IMPLEMENT WEIGHTING MANUALLY (11/02/2020)
   =========================================
   Since all observations are counted for a given bin (with weight 1), we can
   instead sum up the provided weights and divide by the sum of weights afterwards.
  */

  if (aweight!="") {
    // weights corresponding to binnum
    st_view(weight, ., aweight, tousevar)
  ...
  for (i = 1; i <= rn; i++) {
    cellnum = binnum[i]
	if (aweight!="") {
	  cellval[cellnum] = cellval[cellnum] + (weight[i]*rn/weightsum)
    }
    else {
