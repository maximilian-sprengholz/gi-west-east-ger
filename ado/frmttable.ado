...
{	// write T.Body to file w/ Tex formatting
	...
	fput(fh,"") // table begin auskommentiert: "\begin{tabular}{"+TabularCols(vlines,coljust,T)+"}"
	lastspace = spaces[br,2]			// separate last aft space
	frstcol = (""\spaces[1..br-1,2])+ParseHLines(hlines,T)+spaces[,1] // xxx: +ParseHLines(hlines,T)+spaces[,1] : offset bef & aft spaces
	...
	if (noblankrows) bodrows = select(bodrows,	///
		rowsum(T.Body:==""):!=cols(T.Body))
	for (i=1;i<=rows(bodrows);i++) fput(fh, bodrows[i])
	lastline = lastspace // + (hlines[br+1]=="1")*"\hline" + "\end{tabular}\\"	xxx
...
{	// convert hlines to hlcol column vector
	if (hlines=="") hlines = "{0};1{0}" // 1{0};1{0}1
	sects = T.SectRows	//														xxx
	sects[cols(sects)] = sects[cols(sects)]+1 // hline after last row
	hlines = ParseLines(hlines,"hlines",sects,"01")
	return((subinstr(subinstr(hlines,"0",""),"1","\hline ")[1..cols(hlines)-1])') // xxx
}
